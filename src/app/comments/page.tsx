...
export default function CommentsPage() {
  ...
  const shouldFetch = user && activeTab !== "analysis";
  const { data: commentsData, error: commentsError, mutate } = useSWR<Comment[]>(
    shouldFetch ? `/api/comments?month=${selectedMonth}&year=${selectedYear}` : null,
    fetcher,
    {
  ...
  const isFetching = shouldFetch && !commentsData && !commentsError;

  useEffect(() => {
    if (commentsError) showToast(commentsError.message, "error");
  }, [commentsError]);
  ...
  const handleUpdate = async (id: string, updates: Partial<Comment>) => {
    try {
      // Optimistic update
      mutate(
        (prev) => prev ? prev.map(c => c.id === id ? { ...c, ...updates } : c) : [],
        false
      );
  ...
  const filteredComments = useMemo(() => {
    return tabFilteredComments.filter(c => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, officeFilter, activeTab]);
                  ...
                  <tr className="bg-background/80 backdrop-blur-md border-b-2 border-border-strong">
                    ...
                    {activeTab === "action" && (
                      <>
                        ...
                        <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-on-surface/40 text-center">Updated At</th>
                      ...
                      <tr key={comment.id} className="hover:bg-on-surface/[0.015] transition-all group">
                        ...
                        {activeTab === "action" && (
                          <>
                            ...
                            <td className="px-5 py-3.5 text-center">
                              <span className="text-[10px] font-bold text-on-surface/50 truncate max-w-[150px] inline-block" title={comment.actionPlan || "None"}>
                                {comment.actionPlan || "None"}
                              </span>
                            </td>
                            <td className="px-5 py-3.5 text-center">
                              <span className="text-[10px] font-bold text-on-surface/50 whitespace-nowrap">
                                {comment.status !== "Pending" || comment.actionPlan || comment.expectedOutcome ? (
                                  (comment.updatedAt || comment.date) ? new Date(comment.updatedAt || comment.date).toLocaleDateString('en-US', {
                                    month: 'short',
                                    day: 'numeric',
                                    year: 'numeric'
                                  }) : "N/A"
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isActionNeeded ? "Management Assignment" : "Feedback Analysis"}>
      <div className="space-y-4 p-1">
        <div className="flex flex-wrap items-center gap-4 bg-on-surface/5 px-4 py-3 rounded-2xl border border-on-surface/5">
          <div className="flex items-center gap-2"><span className="text-[9px] font-black text-on-surface/30 uppercase tracking-[0.2em]">Office:</span><span className="text-xs font-black text-primary">{comment.office}</span></div>
          <div className="flex items-center gap-2"><span className="text-[9px] font-black text-on-surface/30 uppercase tracking-[0.2em]">Period:</span><span className="text-xs font-bold text-on-surface/60">{comment.month || "Unknown"}</span></div>
          <div className="flex items-center gap-2"><span className="text-[9px] font-black text-on-surface/30 uppercase tracking-[0.2em]">Submitted:</span><span className="text-xs font-bold text-on-surface/60">{comment.date ? new Date(comment.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : "N/A"}</span></div>
          <div className="flex items-center gap-2 ml-auto"><span className="text-[9px] font-black text-on-surface/30 uppercase tracking-[0.2em]">Class:</span><SentimentBadge sentiment={comment.sentiment} /></div>
        </div>
        <div className="space-y-1.5">
          <div className="px-4 py-3 bg-on-surface/[0.02] border border-on-surface/5 rounded-xl italic text-sm font-medium text-on-surface/80 leading-relaxed shadow-inner">
            <span className="text-[9px] font-black text-on-surface/30 uppercase tracking-[0.2em] mr-2">Feedback:</span>
            &ldquo;{comment.commentText}&rdquo;
          </div>
        </div>
        {isActionNeeded && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2"><label className="text-[10px] font-black text-on-surface/30 uppercase tracking-[0.2em] ml-2">Action Taken</label><textarea value={localValues.actionPlan} onChange={(e) => setLocalValues({ ...localValues, actionPlan: e.target.value })} className="w-full bg-white border border-on-surface/10 rounded-2xl p-4 text-sm font-semibold min-h-[100px] outline-none focus:ring-4 focus:ring-primary/10 transition-all shadow-sm" placeholder="Details of action taken..." /></div>
        ...
        {isActionNeeded && (
          <div className="flex items-center justify-between p-4 bg-on-surface/5 rounded-2xl border border-on-surface/5">
            <label className="text-[10px] font-black text-on-surface/30 uppercase tracking-[0.2em]">Status</label>
            <div className="flex gap-2">{(["Pending", "Ongoing", "Resolved"] as Status[]).map((st) => (<button key={st} onClick={() => setLocalValues({ ...localValues, status: st })} className={clsx("px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", localValues.status === st ? "bg-primary text-white shadow-md shadow-primary/20" : "bg-white text-on-surface/30 hover:bg-gray-200")}>{st}</button>))}</div>
          </div>
        )}
        <div className="flex justify-end gap-3 pt-4 border-t border-on-surface/5">
          <button onClick={onClose} className="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-on-surface/40 hover:text-on-surface/60 transition-colors">Close</button>
          {isActionNeeded ? (<Button onClick={() => onSave(comment.id, localValues)} className="px-8 shadow-md shadow-primary/30">Commit Changes</Button>) : (<Button onClick={onClose} className="px-8">Return</Button>)}
        </div>
      </div>
    </Modal>
  );
