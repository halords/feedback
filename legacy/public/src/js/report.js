...
window.onload = async function() {
  ...
  if (!loggedInUser) {
    window.location.href = 'login.html';  // Redirect to login page if not logged in
      ...
      setOptions();
      await getData(loggedInUser.offices, months[currentMonth], currentYear);  // Fetch user data if logged in
      await summaryReport(months[currentMonth], currentYear);
...
async function summaryReport(month, year){
  ...
  try{
    ...
    Object.keys(results).forEach((key) => {
      if(results[key].overrate !== "N/A"){
        genCount += 1;
        genRating += Number(results[key].overrate.replace('%', ''));
        // console.log(genRating, genCount, results[key].overrate);
      }
      const totalResponses = Number(results[key].offline) + Number(results[key].online);
      respondentsNum += totalResponses;
      registeredNum += Number(results[key].visitor);
      maleNum += Number(results[key].gender.Male);
      femaleNum += Number(results[key].gender.Female);
      lgbtqNum += Number(results[key].gender.LGBTQ);
      undefinedNum += Number(results[key].gender.Others);
      positiveNum += Number(results[key].comments.positive?.length);
      negativeNum += Number(results[key].comments.negative?.length);
      suggestNum += Number(results[key].comments.suggestions?.length);
      html += `
        <tr>
    ...
    html += `
      <tr>
        ...
        <td><strong>${respondentsNum === 0 ? "" : respondentsNum}</strong></td>
        <td><strong>${registeredNum === 0 ? "" : registeredNum}</strong></td>
        <td><strong>${maleNum === 0 ? "" : maleNum}</strong></td>
        <td><strong>${femaleNum === 0 ? "" : femaleNum}</strong></td>
        <td><strong>${lgbtqNum === 0 ? "" : lgbtqNum}</strong></td>
        <td><strong>${undefinedNum === 0 ? "" : undefinedNum}</strong></td>
        <td colspan="3"></td>
        <td><strong>${Number((genRating / genCount).toFixed(2))}%</strong></td>
        <td><strong>${positiveNum === 0 ? "" : positiveNum}</strong></td>
        <td><strong>${negativeNum === 0 ? "" : negativeNum}</strong></td>
        <td><strong>${suggestNum === 0 ? "" : suggestNum}</strong></td> 
      </tr>
    `;
    html += `</tbody>
          </table>
...
async function init(offices) {
  ...
  // console.log(monthlyData[years.value], years.value);
  ...
  mont?.addEventListener('change', async(e) => {
    ...
    for (let i = 0; i < monthNames.length; i++) {
      ...
      if (values.includes(month)) {
        const monthR = await changeFilter(month, intM);
        // console.log(monthR);
        let count2 = 0;
        montS = month;
        Object.keys(monthR).forEach((key) => {
          if( monthR[key].overrate !== 'N/A'){
            count2 +=1;
            overallRate2 += parseFloat(monthR[key].overrate.replace("%", ""));
            // console.log(monthR[key].overrate);
            allResponse2 += monthR[key].online + monthR[key].offline;
            // console.log(allResponse);
          }
        });
        const pushRate = {
          overallRate : parseFloat((overallRate2/count2).toFixed(2))
        };
        // console.log(month, pushRate);
        const responses = {
          responses : allResponse2
        }
        // console.log(month, responses);
        if(!monthlyData2[intM]){
          monthlyData2[intM] = [];
        }
        if(!responseData2[intM]){
          responseData2[intM] = [];
        }
        monthlyData2[intM].push({montS, pushRate});
        responseData2[intM].push({montS, responses});
      }
    }

    console.log(monthlyData2);
    ...
    renderChart(monthlyData2[intM], 'monthlyRating', intM.toString(), monthDataC, line);
    renderChart(responseData2[intM], 'monthlyGender', intM.toString(), responseC, line);
  ...
  years?.addEventListener('change', async(e) => {
    ...
    // console.log(years.value, "This is a trial run");
    ...
    Object.keys(dataY).forEach((key) => {
      ...
      const secondData = {
        ...
        'Systems and Procedures' : parseFloat(dataY[key].sysrate.replace("%", "")),
        'Staff Service' : parseFloat(dataY[key].staffrate.replace("%", ""))
      }
      const thirdData = {
