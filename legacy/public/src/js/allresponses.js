window.onload = function() {
  ...
  let coms = [];
    ...
    async function fetchUserData() {
      ...
      "OPAg", "OPVet", "PGENRO", "PIO", "LUPTO", "PICTO", "BACSD", "LEEIPO", 
  ...
  if (currentYear > baselineYear) {
    ...
    for (let month = 1; month <= 12; month++) {
      filterMonths.forEach(filterMonth => {
        ...
        option2.selected = month - 1 === currentMonth; // Fix: `month-1` to match 0-indexed month
    ...
    for (let month = baselineMonth; month <= currentMonth + 1; month++) {
      filterMonths.forEach(filterMonth => {
        ...
        option2.value = month;
        option2.innerText = months[month - 1];
        option2.selected = month - 1 === currentMonth;
        filterMonth.appendChild(option2);
      });
    }
  }
