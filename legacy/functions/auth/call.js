...
async function getUserData(db, offices) {
    ...
    try {
      ...
      if (offices.includes('All')) {
        try {
          const snapshot = await db.collection('Responses').orderBy('Date', 'desc').get();
          if (!snapshot.empty) {
...
async function getOnlineReport(db, offices, month, year) {
  const batchSize = 10;  // Lowered to 10 for maximum compatibility with all Firestore environments
    ...
    try {
      ...
      snapshot.forEach(doc => {
        ...
        if (gender === 'Male') {
          ...
          if (results[office]) results[office].gender.LGBTQ++;
        } else {
          if (results[office]) results[office].gender.Others++;
        }

        if (results[office]) {
          results[office].collection = results[office].gender.Male + 
                                       results[office].gender.Female + 
                                       results[office].gender.LGBTQ + 
                                       results[office].gender.Others;
        }
...
async function getOfflineReport(db, offices, month, year) {
  const batchSize = 10;  // Lowered to 10 for maximum compatibility
...
async function generatePDF(formData) {
  ...
  const fieldMap = {
    ...
    'clientType.Government (Employee or another Agency)': 'GOVERNMENT',
    'cc1.Yes': 'YES',
...
async function fetchDashboard(db, months){
  // const snapshot = await db.collection('Responses').where('Office', 'in', offices).get(); 
  try{
    if (!months?.months?.month) throw new Error("Missing month parameters");
    const data = [];
    for (let i = 0; i < months.months.month.length; i++) {
        const month = months.months.month[i];
        const year = months.months.year[i];
        const offices = months.months.offices || [];
        
        const result = await analyzeResponsesByOffice(db, offices, month, year);
        data.push(result);
    }
    // console.log(data);
